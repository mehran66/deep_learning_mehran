...
def fine_tune(assess_model=True):
    ...
    # define callbacks
    reduce_lr, early_stopping, model_checkpoint, lr_scheduler, tensorboard_callback = callbacks(model_check_points, main_model_name, backbone_model_name, tboard_dir, plot_dir, epochs, lr, stage = 'fine_tune')
    print("\ncallbacks were generated: reduce_lr, early_stopping, model_checkpoint, lr_scheduler, tensorboard_callback")
    ...
    if assess_model:
        ...
        loaded_saved_model = tf.keras.models.load_model(os.path.join(model_check_points, f'{main_model_name}_{str(backbone_model_name)}_fine_tune_val_IoU_max.hdf5'), custom_objects=custom_objects)
