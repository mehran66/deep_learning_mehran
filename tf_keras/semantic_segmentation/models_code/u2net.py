# https://github.com/baudcode/tf-semantic-segmentation/blob/master/tf_semantic_segmentation/models/u2net.py
...
def conv(x, filters=3, dirate=1, kernel_size=3, l2=None, activation=None):
...
def build_u2net(n_classes, inputs, activation):
    ...
    for i, dout in enumerate(reversed(decoder_layers)):
        s = conv(dout, n_classes, dirate=1)
        side = upsample(s, pow(2, i))
        side_layers.append(side)

    out = conv(tf.concat(side_layers, axis=-1), n_classes, kernel_size=1, activation=activation)
